# Automax per-plugin smoke runbooks

Per-plugin runbooks only. No combined runbook, no reused runtime smoke flow, and no separate destructive workflow.

| File | Plugin/family | Substeps |
|---|---:|---:|
| `runbooks/01-alternatives.check.yaml` | `alternatives` | 3 |
| `runbooks/02-apparmor.check.yaml` | `security.apparmor` | 8 |
| `runbooks/03-archive.check.yaml` | `archive` | 6 |
| `runbooks/04-storage-usage.check.yaml` | `storage.usage` | 2 |
| `runbooks/05-auditd.check.yaml` | `security.audit` | 8 |
| `runbooks/06-authselect.check.yaml` | `security.authselect` | 1 |
| `runbooks/07-backup.check.yaml` | `backup` | 9 |
| `runbooks/08-storage-fs.check.yaml` | `storage.fs` | 4 |
| `runbooks/09-storage-block.check.yaml` | `storage.block` | 10 |
| `runbooks/10-capability.check.yaml` | `capability` | 1 |
| `runbooks/11-cert.check.yaml` | `security.pki.cert` | 11 |
| `runbooks/12-chrony.check.yaml` | `chrony` | 3 |
| `runbooks/13-cron.check.yaml` | `cron` | 5 |
| `runbooks/14-db.check.yaml` | `db` | 5 |
| `runbooks/15-download.check.yaml` | `download` | 1 |
| `runbooks/16-env.check.yaml` | `env` | 1 |
| `runbooks/17-facts.check.yaml` | `facts` | 5 |
| `runbooks/19-firewalld.check.yaml` | `network.firewall.firewalld` | 11 |
| `runbooks/20-fs.check.yaml` | `fs` | 31 |
| `runbooks/21-storage-fstab.check.yaml` | `storage.fstab` | 4 |
| `runbooks/22-group.check.yaml` | `identity.group` | 5 |
| `runbooks/24-hostname.check.yaml` | `hostname` | 1 |
| `runbooks/25-hosts.check.yaml` | `hosts` | 1 |
| `runbooks/26-http.check.yaml` | `http` | 3 |
| `runbooks/27-iptables.check.yaml` | `network.firewall.iptables` | 9 |
| `runbooks/28-journal.check.yaml` | `journal` | 2 |
| `runbooks/29-kernel.check.yaml` | `kernel` | 8 |
| `runbooks/30-limits.check.yaml` | `limits` | 1 |
| `runbooks/31-local.check.yaml` | `local` | 1 |
| `runbooks/32-log.check.yaml` | `log` | 2 |
| `runbooks/33-login.check.yaml` | `login` | 1 |
| `runbooks/34-storage-lvm.check.yaml` | `storage.lvm/storage.fs` | 14 |
| `runbooks/35-mail.check.yaml` | `mail` | 1 |
| `runbooks/36-storage-mount.check.yaml` | `storage.mount` | 6 |
| `runbooks/37-storage-multipath.check.yaml` | `storage.multipath` | 4 |
| `runbooks/38-network.check.yaml` | `network` | 14 |
| `runbooks/39-nftables.check.yaml` | `network.firewall.nftables` | 6 |
| `runbooks/40-pam.check.yaml` | `security.pam/security.authselect` | 14 |
| `runbooks/41-password.check.yaml` | `security.password` | 1 |
| `runbooks/42-pkg.check.yaml` | `pkg` | 18 |
| `runbooks/43-pki.check.yaml` | `security.pki` | 3 |
| `runbooks/44-platform.check.yaml` | `platform` | 1 |
| `runbooks/45-plugin.check.yaml` | `plugin` | 1 |
| `runbooks/46-process.check.yaml` | `process` | 5 |
| `runbooks/47-remote.check.yaml` | `remote` | 1 |
| `runbooks/48-network-dns-facts.check.yaml` | `network.dns` | 2 |
| `runbooks/49-secret.check.yaml` | `security.secret` | 3 |
| `runbooks/50-selinux.check.yaml` | `security.selinux` | 6 |
| `runbooks/51-ssh.check.yaml` | `security.ssh` | 8 |
| `runbooks/52-sshd.check.yaml` | `security.sshd` | 2 |
| `runbooks/53-sudo.check.yaml` | `security.sudo` | 6 |
| `runbooks/55-storage-swap.check.yaml` | `storage.swap` | 4 |
| `runbooks/56-sysctl.check.yaml` | `sysctl` | 7 |
| `runbooks/57-system.check.yaml` | `system` | 1 |
| `runbooks/58-systemctl.check.yaml` | `systemctl` | 12 |
| `runbooks/59-systemd.check.yaml` | `systemd` | 4 |
| `runbooks/60-timedatectl.check.yaml` | `timedatectl` | 3 |
| `runbooks/61-tool.check.yaml` | `tool` | 2 |
| `runbooks/62-transfer.check.yaml` | `transfer` | 4 |
| `runbooks/63-udev.check.yaml` | `udev` | 7 |
| `runbooks/64-ufw.check.yaml` | `network.firewall.ufw` | 6 |
| `runbooks/65-user.check.yaml` | `identity.user` | 11 |
