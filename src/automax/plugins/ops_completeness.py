...
class BlockSizeAssertPlugin(ReadOnlyCommandPlugin):
    name="storage.block.size.check"; description="Check block device size in bytes."; required_params=("device","size"); optional_params=("sudo",)
...
class BlockEmptyAssertPlugin(ReadOnlyCommandPlugin):
    ...
    def manual_commands(self, params: Dict[str, Any], context: ExecutionContext)->list[str]: return [f"test -b {quote(params['device'])} && ! {sudo_prefix(params, default=True)}blkid {quote(params['device'])}"]
