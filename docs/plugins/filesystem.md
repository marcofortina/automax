...
```text
fs.acl          # set or remove ACL entries with optional getfacl backup
fs.acl.get      # read ACLs with getfacl
fs.acl.assert   # read-only assertion for present/absent ACL entries
fs.acl.restore  # restore ACLs from a getfacl backup file
```

Use `fs.acl.get` before changes and `fs.acl.assert` after changes when ACLs are
