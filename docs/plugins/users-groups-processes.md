...
`identity.user.check` and `identity.group.check` are read-only assertions for account prerequisites. `identity.user.lock`, `identity.user.unlock` and `identity.user.password.set` are explicit account state changes and should be reviewed separately from user creation/removal.
